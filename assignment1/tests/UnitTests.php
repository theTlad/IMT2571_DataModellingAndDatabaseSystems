	...
	public function testAddBook()
	{
		...
		for ($i = self::FIXTURE_SIZE; $i < sizeof(self::$TEST_CASES); $i++)
		{
			...
			{	
				...
				//Was auto_increment on id not triggered? (ID should be the same as before)
				$this->assertLessThan($book->id+1, $book->id);
				
				//Was no book added? (The database size is the same as before)
	...
	public function testModifyBook()
	{
		...
		for ($i = self::FIXTURE_SIZE; $i < sizeof(self::$TEST_CASES); $i++)
		{
			...
			else
			{
				...
				//$this->assertNotNull($exception, "testModifyBook: Exception not thrown.\n");
				
				echo "LKJKLJKLJKLJLKJLKJLKJ\n";
				...
				PLEASE READ: Tried to do this part in the end, but I give up this section.
							 No idea how to solve this, phpunit keep giving me errors (not failures).
							(Out- commented code from this section is what I tried to do.)
				*/			
